...
class MealForm(Form):
	...
	# format='%m/%d/%y', 
	date = DateField('Date (mm/dd/yyyy) in the next month', format='%d/%m/%Y', validators=[Required(), DateRange(min=date(currentYear, currentMonth, currentDay), max=date(currentYear, (currentMonth+1) % 12, min(currentDay, 28)))])
