...
class MealForm(Form):
	...
	# format='%m/%d/%y', 
	date = DateField('Date (mm/dd/yyyy) in the next month', format='%m/%d/%Y', validators=[Required(), DateRange(min=date(currentYear, currentDay, currentMonth), max=date(currentYear, min(currentDay, 28), (currentMonth+1) % 12))])
